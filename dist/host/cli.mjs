...
async function createChangeproofHost() {
  ...
  const self = {
    ...
    async activate() {
      registerTool(
        "changeproof_plan",
        async (input) => planTool(host, requireWorkspace(input), {
...
function argValue(name) {
  ...
  const v = argv[i + 1];
  if (v === void 0 || v.startsWith("--")) return void 0;
  return v;
}
async function main() {
  ...
  try {
    if (command === "plan") {
      const result2 = await cp.tools.invoke("changeproof_plan", { workspace });
      process.stdout.write(canonicalJsonStringify(result2) + "\n");
    ...
    if (command === "verify") {
      if (!approveAll) {
        ...
        const preview = plan.data.preview;
...
`);
      ...
      const result2 = await cp.tools.invoke("changeproof_verify", { workspace, approvalIntent: "approve" });
      process.stdout.write(canonicalJsonStringify(result2) + "\n");
      const status = result2.ok && result2.data && isVerdictStatus(result2.data.verdict?.status) ? result2.data.verdict.status : null;
...
var invokedAs = (process.argv[1] ?? "").replace(/\\/g, "/");
var invokedDirectly = /\/cli\.(ts|mjs)$/.test(invokedAs) && !process.env["DSH_CHANGEPROOF_IMPORTED"];
if (invokedDirectly) {
  main().then((code) => {
